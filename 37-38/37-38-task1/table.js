   ...
   //表格
    ...
    for (var i = 1; i < list.length; i++) {
    	if((i==list.length-1)||(list[i][0]!=list[i+1][0])){
            ...
            Trlist[i-j].childNodes[0].style.display='none';
    		}
...
function addinput(fathernode,content){
    
    var newinput=document.createElement('input');
    newinput.setAttribute('type','text');
    newinput.setAttribute('style','width:50px');
    newinput.setAttribute('onblur','addinputjudge()');
    
    if(fathernode.tagName!='TH'){
    newinput.setAttribute('name','forminput');
    }
    newinput.value=content;
    fathernode.appendChild(newinput);
